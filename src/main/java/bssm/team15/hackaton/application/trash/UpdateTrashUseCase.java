...
public class UpdateTrashUseCase {
    ...
    @Transactional
    public void update(User user, Long id, MultipartFile imageData, String location) {
        ...
        if (!trash.getUserId().equals(user.getId())) throw new ResponseStatusException(
                HttpStatus.FORBIDDEN,
                ""
        );
        trash.certify(location);
        try {
    ...
    private void saveImage(Long id, MultipartFile imageData) throws IOException {
        ...
        Path target = dir.resolve(id + "_2" + ".jpeg");
